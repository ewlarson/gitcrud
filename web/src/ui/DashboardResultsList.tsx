    ...
    return (
        <div className="space-y-4">
            {resources.map((r, index) => (
                <div key={r.id} className="group relative grid grid-cols-[auto_1fr] sm:grid-cols-[auto_1fr_auto] gap-4 rounded-lg border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-4 hover:border-gray-300 dark:hover:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-900/60 transition-colors shadow-sm hover:shadow-md">
                    ...
                    <div className="col-span-1 sm:col-span-1 grid grid-cols-[1fr] sm:grid-cols-[auto_1fr] gap-4 w-full">
                        {/* Thumbnail */}
                        <div className="hidden sm:flex w-40 h-40 bg-gray-100 dark:bg-slate-950 rounded border border-gray-200 dark:border-slate-800 items-center justify-center overflow-hidden flex-shrink-0">
                            {thumbnails[r.id] ? (
                    ...
                    <div className="w-40 hidden sm:flex flex-col gap-2">
                        <div className="h-40 w-full bg-gray-100 dark:bg-slate-950 rounded border border-gray-200 dark:border-slate-800 overflow-hidden relative">
                            {mapUrls[r.id] ? (
                                <img
                                    src={mapUrls[r.id]!}
                                    alt="Location Map"
